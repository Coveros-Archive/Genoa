...
func (wH WebhookHandler) syncReleaseWithGithub(owner, repo, branch, SHA, releaseFile string, gitFactory git.Git, isRemovedFromGithub bool) {
	...
	if branchToFollow, ok := hrFromGit.Annotations[utils.GitBranchToFollowAnnotation]; ok && branchToFollow != "" {
		if branchToFollow != branch {
			wH.Logger.Info(fmt.Sprintf("Skipping sync for %v from %v/%v, follow-git-branch '%v' does not match current branch '%v'", hrFromGit.GetName(), owner, repo, branchToFollow, branch))
			return
		}
	}
	...
	if errCreatingHR != nil {
		wH.Logger.Info(fmt.Sprintf("%v/%v failed to create release : %v", hrFromGit.GetNamespace(), hrFromGit.GetName(), errCreatingHR))
	}
	wH.Logger.Info(fmt.Sprintf("Successfully created %v/%v Release", hrFromGit.GetNamespace(), hrFromGit.GetName()))
	...
	if !specInSync || !labelsInSync || !annotationsInSync {
		...
		if errUpdating := wH.Client.Update(context.TODO(), hrFromCluster); errUpdating != nil {
			wH.Logger.Error(errUpdating, fmt.Sprintf("Failed to apply release from %v/%v - %v", owner, repo, hrFromGit.GetName()))
			return
		}

		wH.Logger.Info(fmt.Sprintf("Updated release from %v/%v - %v/%v", owner, repo, hrFromGit.GetNamespace(), hrFromGit.GetName()))
	}
